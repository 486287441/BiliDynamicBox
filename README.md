...
- 顶部工具栏提供垃圾箱入口
- 使用 DeepSeek V4 Flash 按标题将视频归为「知识」或「娱乐」，支持「全部 / 知识 / 娱乐」切换
- AI 分类结果会复用，不会在每次刷新时重新判断；超过 180 天或超出 4000 条的旧标记会自动清理
...
5. 首次使用 AI 筛选时点击工具栏的「设置 AI」，填入 DeepSeek API Key。Key 仅保存在扩展自己的 `chrome.storage.local` 中，不会写入网页、本仓库或构建产物。
...
- DeepSeek 请求由 Manifest V3 后台 Service Worker 发起，内容页只能提交标题并读取分类结果，不能读取保存的 API Key。浏览器本地扩展存储并非系统级加密保险箱，请只在可信设备上使用。
